...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch actual route_id to prevent foreign key constraints when inserting into logs
    $stmt_route = $pdo->prepare("SELECT route_id FROM applications WHERE id = ?");
    $stmt_route->execute([$application_id]);
    $current_route_id = (int) $stmt_route->fetchColumn();
    ...
    elseif ($action === 'approve') {
        ...
        if ($validity_months) {
            try {
                ...
                $stmt_log = $pdo->prepare("INSERT INTO route_validation_logs (application_id, route_id, is_valid, validation_notes) VALUES (?, ?, 1, ?)");
                $stmt_log->execute([$application_id, $current_route_id, "Route validation and documentation criteria completed. Pass issued."]);
        ...
        <div class="detail-card">
            ...
            ?>">
                ...
                <div class="doc-actions">
                    <?php 
                        $doc_url = str_starts_with($app['college_id_doc'] ?? '', 'uploads/') ? '../Student-Dashboard/' . $app['college_id_doc'] : '../Student-Dashboard/uploads/' . $app['college_id_doc'];
                    ?>
                    <a href="<?php echo htmlspecialchars($doc_url); ?>" target="_blank" class="btn btn-secondary btn-sm" title="View Document File">
            ...
            ?>">
                ...
                <div class="doc-actions">
                    <?php 
                        $doc_url = str_starts_with($app['photograph_doc'] ?? '', 'uploads/') ? '../Student-Dashboard/' . $app['photograph_doc'] : '../Student-Dashboard/uploads/' . $app['photograph_doc'];
                    ?>
                    <a href="<?php echo htmlspecialchars($doc_url); ?>" target="_blank" class="btn btn-secondary btn-sm">
                        <i class="fa-solid fa-up-right-from-square"></i> View File
                    </a>
            ...
            ?>">
                ...
                <div class="doc-actions">
                    <?php 
                        $doc_url = str_starts_with($app['address_proof_doc'] ?? '', 'uploads/') ? '../Student-Dashboard/' . $app['address_proof_doc'] : '../Student-Dashboard/uploads/' . $app['address_proof_doc'];
                    ?>
                    <a href="<?php echo htmlspecialchars($doc_url); ?>" target="_blank" class="btn btn-secondary btn-sm">
                        <i class="fa-solid fa-up-right-from-square"></i> View File
                    </a>
