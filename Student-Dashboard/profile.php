...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($full_name) || empty($phone) || empty($college_name)) {
        ...
        if (empty($error)) {
            // Update students table (uses mobile, department, class instead of phone, course, year_of_study)
            $q = "UPDATE students SET mobile=?, address=?, college_name=?, department=?, class=?, profile_pic=? WHERE id=?";
            $s = mysqli_prepare($conn, $q);
            mysqli_stmt_bind_param($s, 'ssssssi', $phone, $address, $college_name, $course, $year_of_study, $profile_pic, $student_id);
            
            if (mysqli_stmt_execute($s)) {
                // Also update the full_name in the users table
                $user_id = $student['user_id'];
                $q2 = "UPDATE users SET full_name=? WHERE id=?";
                $s2 = mysqli_prepare($conn, $q2);
                mysqli_stmt_bind_param($s2, 'si', $full_name, $user_id);
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="year_of_study" class="form-select">
                                            <option value="">Select</option>
                                            <?php for ($y = 1; $y <= 4; $y++): ?>
                                            <option value="<?php echo $y; ?>" <?php echo ($student['class'] ?? '') == $y ? 'selected' : ''; ?>>
