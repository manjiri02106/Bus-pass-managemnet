...
// Fetch active routes
$routes_query = "SELECT * FROM routes WHERE status = 'Active' ORDER BY source, destination";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_application'])) {
    ...
    if (!isset($_POST['route_id']) || empty($pass_type)) {
        ...
        if ($route_id === '0' || $route_id === 0) {
            ...
            $route_number = 'CUST-' . strtoupper(substr(uniqid(), -6));
            $stops = 'Direct';
            $distance = 0;
            $bus_number = 'TBD';

            $insert_q = "INSERT INTO routes (route_number, source, destination, stops, distance, bus_number, status) VALUES (?, ?, ?, ?, ?, ?, 'active')";
            $insert_s = mysqli_prepare($conn, $insert_q);
            mysqli_stmt_bind_param($insert_s, 'ssssds', $route_number, $custom_source, $custom_dest, $stops, $distance, $bus_number);
        ...
        if (empty($error)) {
            ...
            if (!$route) {
                ...
                $base_fare = isset($custom_fare) && $custom_fare > 0 ? $custom_fare : ($route['fare'] ?? getDailyFareFromCSV($route['distance'] ?? 0, 10));
                ...
                if ($upload_ok) {
                    ...
                    // Insert bus pass with payment_status = Success
                    $query = "INSERT INTO applications (application_no, student_id, route_id, pass_type, college_id_doc, photograph_doc, address_proof_doc, fee, valid_from, valid_until, payment_status, routing_dept) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Success', '')";
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        $pay_q = "INSERT INTO payments (application_id, student_id, amount, payment_method, transaction_id, payment_status, payment_date) VALUES (?, ?, ?, ?, ?, 'Success', NOW())";
                        $pay_s = mysqli_prepare($conn, $pay_q);
                        mysqli_stmt_bind_param($pay_s, 'iidss', $pass_id, $student_id, $fee, $payment_method, $transaction_id);
                        mysqli_stmt_execute($pay_s);

                        // Create notification
                        $notif_q = "INSERT INTO notifications (student_id, title, message, type) VALUES (?, 'Application Submitted & Paid', 'Your bus pass application (Ref: $app_no) has been submitted successfully and payment of ₹$fee is received.', 'success')";
                        ...
                        redirect('/my_applications.php', 'Application and payment successful!', 'success');
                    } else {
...
<body>
        ...
        <div class="fade-in">
            ...
            <div class="card shadow-sm">
                <div class="card-body p-4">
                    <form method="POST" action="" enctype="multipart/form-data" id="applyPassForm"
                        ...
                        <div class="step-content" id="step1">
                            ...
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label">Destination Stop <span
                            ...
                            <div class="row g-3">
                                <div class="col-md-8">
                                    <label class="form-label">Matching Bus Route <span
                                            class="text-danger">*</span></label>
                                    ...
                                    <div id="timing_details" class="mt-2 p-2 bg-light rounded border"
                            ...
                            <div id="pass_summary_card" class="mt-4" style="display:none;">
                                <div class="card border-primary">
                                    <div
                                        ...
                                        <span><i class="bi bi-receipt me-2"></i><strong>Pass Price
                                                Summary</strong></span>
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div id="selected_price_highlight" class="mt-3 p-3 rounded"
                                            style="background:#e8f5e9;display:none;">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <div class="text-muted small">Total Amount to Pay</div>
                                                    <div id="selected_price_text" class="fs-4 fw-bold text-success">—
                                                    </div>
                                                </div>
                                                <div class="text-end">
                        ...
                        <div class="step-content" id="step2" style="display:none;">
                            ...
                            <div class="row g-4">
                                <div class="col-md-4">
                                    <label class="form-label fw-bold">College ID Card <span
                                            class="text-danger">*</span></label>
                                    <div class="upload-area"
                                        onclick="document.getElementById('college_id_doc').click()">
                                        ...
                                        <img id="preview_college_id" class="img-fluid mt-2 rounded"
                                ...
                                <div class="col-md-4">
                                    <label class="form-label fw-bold">Passport Photo <span
                                            class="text-danger">*</span></label>
                                    <div class="upload-area" onclick="document.getElementById('photo_doc').click()">
                                        ...
                                        <img id="preview_photo" class="img-fluid mt-2 rounded"
                                            style="max-height:120px;display:none;">
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <label class="form-label fw-bold">Address Proof <span
                                            class="text-danger">*</span></label>
                                    <div class="upload-area"
                                        onclick="document.getElementById('address_proof_doc').click()">
                                        <div class="upload-icon"><i class="bi bi-house-door"></i></div>
                                        <p class="mb-1">Click to upload Address Proof</p>
                                        <span class="file-name text-muted small">JPG, PNG or PDF</span>
                                        <img id="preview_address" class="img-fluid mt-2 rounded"
                                            style="max-height:120px;display:none;">
                                    </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="card bg-light">
                                        <div class="card-body">
                                            <h6 class="fw-bold"><i
                                                    class="bi bi-receipt me-2 text-primary"></i>Application Summary</h6>
                                            <table class="table table-sm mb-0">
                                                <tr>
                                                    <td class="text-muted">Source Stop:</td>
                                                    <td class="fw-bold" id="review_source">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Destination Stop:</td>
                                                    <td class="fw-bold" id="review_dest">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Route:</td>
                                                    <td class="fw-bold" id="review_route">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Pass Type:</td>
                                                    <td class="fw-bold text-success" id="review_pass_type">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Total Fee:</td>
                                                    <td class="fw-bold text-primary fs-5" id="review_fee">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Valid From:</td>
                                                    <td id="review_valid_from">-</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">Valid Until:</td>
                                                    <td id="review_valid_until">-</td>
                                                </tr>
                                            </table>
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <table class="table table-sm mb-0">
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo htmlspecialchars($student['college_name'] ?? ''); ?>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="text-muted">College ID:</td>
                                                    <td><?php echo htmlspecialchars($student['college_id_number'] ?? ''); ?>
                                                    </td>
                                                </tr>
                                                <tr>
                                        ...
                                        <div class="card-body">
                                            <div class="btn-group w-100 mb-4" role="group">
                                                <input type="radio" class="btn-check" name="payment_method"
                                                    id="pay_card" value="Card" autocomplete="off" checked
                                                    onchange="togglePayment('card')">
                                                <label class="btn btn-outline-primary" for="pay_card"><i
                                                        class="bi bi-credit-card-2-front me-2"></i>Card</label>

                                                <input type="radio" class="btn-check" name="payment_method" id="pay_upi"
                                                    value="UPI" autocomplete="off" onchange="togglePayment('upi')">
                                                <label class="btn btn-outline-primary" for="pay_upi"><i
                                                        class="bi bi-qr-code-scan me-2"></i>UPI</label>

                                                <input type="radio" class="btn-check" name="payment_method" id="pay_net"
                                                    value="NetBanking" autocomplete="off"
                                                    onchange="togglePayment('net')">
                                                <label class="btn btn-outline-primary" for="pay_net"><i
                                                        class="bi bi-bank me-2"></i>Net Banking</label>
                                            </div>
                                            ...
                                            <div id="payment_upi" style="display:none;" class="text-center">
                                                <p class="text-muted mb-3">Scan the QR code to pay using any UPI App</p>
                                                <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=upi://pay?pa=9960024125@fam&pn=StudentBusPass"
                                                    alt="UPI QR" class="img-thumbnail mb-3">
                                                <p class="fw-bold">UPI ID: 9960024125@fam</p>
                                            </div>

                                            <div id="payment_net" style="display:none;">
                                    ...
                                    <div class="card bg-light border-primary">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between mb-2">
                                                ...
                                                <span class="fw-bold" id="pay_summary_type">-</span>
                                            </div>
                                            <div class="d-flex justify-content-between mb-2">
                                                <span class="text-muted">Route</span>
                                                <span class="fw-bold text-truncate ms-3" id="pay_summary_route"
                                                    style="max-width: 200px;">-</span>
                                            </div>
                                            <hr>
                                            <div class="d-flex justify-content-between align-items-center mb-4">
                                                <span class="fw-bold">Total Amount Payable</span>
                                                <h3 class="fw-bold text-primary mb-0" id="pay_summary_fee">₹0.00</h3>
                                            </div>
                                            <div class="d-grid gap-2">
                                                <button type="submit" name="submit_application"
                                                    class="btn btn-primary btn-lg">
                                                    <i class="bi bi-lock-fill me-1"></i> Pay & Submit
                                                </button>
    ...
    <script>
        ...
        function updateReviewSummary() {
            ...
            document.getElementById('pay_summary_type').textContent = passType ? passType + ' Pass' : '-';
            document.getElementById('pay_summary_route').textContent = selectedOption && selectedOption.value ? selectedOption.text.trim() : '-';
            document.getElementById('pay_summary_fee').textContent = feeVal ? '₹' + parseFloat(feeVal).toLocaleString('en-IN') : '-';
        }

        function togglePayment(method) {
            document.getElementById('payment_card').style.display = 'none';
            document.getElementById('payment_upi').style.display = 'none';
            document.getElementById('payment_net').style.display = 'none';
            document.getElementById('payment_' + method).style.display = 'block';
        }
