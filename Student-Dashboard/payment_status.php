...
$q = "SELECT p.*, a.application_no, a.pass_type, a.fee as pass_fee, a.status as pass_status,
...
// Also get pending payments from applications
$q2 = "SELECT a.*, r.source, r.destination 
       FROM applications a 
       JOIN routes r ON a.route_id = r.id 
       WHERE a.student_id = ? AND a.payment_status = 'Pending' AND a.status IN ('submitted', 'under_verification', 'approved')
                            ...
                            <tbody>
                                <?php if (mysqli_num_rows($payments) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($p['source'] ?? '') . ' → ' . htmlspecialchars($p['destination'] ?? ''); ?></td>
