...
try {
    // Update applications
    $update_q = "UPDATE applications 
                 SET payment_status = 'Paid' 
                 WHERE id = ?";
    $update_s = mysqli_prepare($conn, $update_q);
    mysqli_stmt_bind_param($update_s, 'i', $pass_id);
    mysqli_stmt_execute($update_s);

    // Insert into payments table
    $insert_payment_q = "INSERT INTO payments (application_id, student_id, amount, payment_method, transaction_id, payment_status, payment_date) 
                         VALUES (?, ?, ?, ?, ?, 'Success', ?)";
    ...
    $_SESSION['payment_success'] = [
        ...
        'route_number' => $decoded_pass['route_number'],
