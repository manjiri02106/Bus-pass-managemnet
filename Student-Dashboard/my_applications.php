...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'cancel') {
    $cancel_id = (int)$_POST['id'];
    $q = "UPDATE applications SET status = 'rejected' WHERE id = ? AND student_id = ? AND status IN ('submitted', 'under_verification')";
    $s = mysqli_prepare($conn, $q);
    mysqli_stmt_bind_param($s, 'ii', $cancel_id, $student_id);
                        ...
                        <tbody>
                            <?php if (mysqli_num_rows($applications) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <small class="text-muted"><?php echo htmlspecialchars($app['source']); ?> → <?php echo htmlspecialchars($app['destination']); ?></small>
                                    </td>
                                    <td><?php echo $app['pass_type'] ?? 'Monthly'; ?></td>
                                    <td>₹<?php echo number_format($app['fee'] ?? 0, 2); ?></td>
                                    <td><?php echo !empty($app['valid_from']) ? date('d-m-Y', strtotime($app['valid_from'])) : '-'; ?></td>
                                    <td><?php echo !empty($app['valid_until']) ? date('d-m-Y', strtotime($app['valid_until'])) : '-'; ?></td>
                                    <td>
                                        <?php
                                        $status_badges = [
                                            'submitted' => 'badge-pending bg-secondary',
                                            'under_verification' => 'badge-pending bg-warning',
                                            'approved' => 'badge-approved bg-success',
                                            'rejected' => 'badge-rejected bg-danger',
                                            'correction_required' => 'bg-info',
                                            'expired' => 'badge-expired bg-dark',
                                            'renewed' => 'badge-approved bg-primary'
                                        ];
                                        $badge_class = $status_badges[$app['status']] ?? 'badge-pending bg-secondary';
                                        ?>
                                    ...
                                    <td>
                                        <?php
                                        $payment_class = match($app['payment_status'] ?? 'Pending') {
                                            'Paid', 'Success' => 'payment-paid text-success',
                                            'Pending' => 'payment-pending text-warning',
                                            'Failed' => 'payment-failed text-danger',
                                            'Refunded' => 'text-info',
                                            default => ''
                                        };
                                        ?>
                                        <span class="<?php echo $payment_class; ?>"><?php echo $app['payment_status'] ?? 'Pending'; ?></span>
                                    </td>
                                    <td><small><?php echo date('d-m-Y', strtotime($app['submission_date'])); ?></small></td>
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            ...
                                            <?php if (($app['payment_status'] ?? 'Pending') === 'Pending'): ?>
                                            <a href="<?php echo BASE_URL; ?>/payment.php?pass_id=<?php echo $app['id']; ?>" 
                                               class="btn btn-outline-success">
                                                <i class="bi bi-credit-card"></i> Pay Now
                                            </a>
                                            <?php endif; ?>
                                            <?php if (in_array($app['status'], ['submitted', 'under_verification'])): ?>
                                            <form method="POST" style="display:inline;">
                                                <input type="hidden" name="action" value="cancel">
                                                <input type="hidden" name="id" value="<?php echo $app['id']; ?>">
                                                <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Cancel this application?');">
                                                    <i class="bi bi-x-lg"></i>
                                                </button>
                                            </form>
                                            <?php endif; ?>
                                            <?php if (($app['payment_status'] ?? '') === 'Success' || ($app['payment_status'] ?? '') === 'Paid'): ?>
                                            <a href="<?php echo BASE_URL; ?>/generate_receipt.php?pass_id=<?php echo $app['id']; ?>" 
                                               class="btn btn-outline-secondary">
                                                <i class="bi bi-receipt"></i>
                                            </a>
                                            <?php endif; ?>
                                            <?php if ($app['status'] === 'approved' && in_array($app['payment_status'], ['Paid', 'Success']) && strtotime($app['valid_until']) >= time()): ?>
    ...
    <script>
        function showDetails(app) {
            const badgeClass = {
                ...
                'under_verification': 'badge-pending',
                'approved': 'badge-approved',
                'rejected': 'badge-rejected',
                'correction_required': 'bg-info',
                'expired': 'badge-expired',
                'renewed': 'badge-approved'
                ...
                <div class="row">
                    <div class="col-md-6">
                        <table class="table table-sm">
                            ...
                            <tr><td class="text-muted">Route:</td><td>${app.route_number || (app.source + ' → ' + app.destination)}</td></tr>
